...
import { getMimeType, getSkillNameValidationError, isBinaryFile, isValidSkillName, normalizeFilePath, } from './utils.js';
/**
    ...
    return {
        ...
        load: async ({ config, logger, watcher, parseData, store, generateDigest, renderMarkdown }) => {
            ...
            if (exists && skillFiles.length === 0) {
                logger.warn(`No skills found in "${relativeBasePath}". Skills must contain a SKILL.md file.`);
            }
            /**
             * Loads a single skill from its directory
             */
            async function loadSkill(skillMdPath, oldId) {
                ...
                if (!frontmatter.name || typeof frontmatter.name !== 'string') {
                    logger.error(`Skill "${skillId}" is missing required "name" field in SKILL.md frontmatter`);
                    return;
                }
                if (!frontmatter.description || typeof frontmatter.description !== 'string') {
                    logger.error(`Skill "${skillId}" is missing required "description" field in SKILL.md frontmatter`);
                    return;
                }
                ...
                await Promise.all(allFiles.map((filePath) => limit(async () => {
            ...
            const matchesSkillDir = (filePath) => {
